...
object FullPipeline {
  ...
  def main(args: Array[String]): Unit = {
    ...
    log("\n----- STEP 8: DEDUPLICATE BY job_id -----")
    val dfDedup = dfStep6.dropDuplicates("job_id")
    dfDedup.cache()
    log(s"  Rows after: ${dfDedup.count()}")

    // Step 9: Remove rows with malformed text in formatted_work_type
    log("\n----- STEP 9: REMOVE MALFORMED WORK TYPE ROWS -----")
    log("  Removing rows where formatted_work_type contains unescaped quotes")
    log("  (caused by job titles with special characters misaligning CSV columns)")
    val dfClean = dfDedup.filter(!col("formatted_work_type").contains("\""))
    dfClean.cache()
    val cleanCount = dfClean.count()
    val malformedCount = dfDedup.count() - cleanCount
    log(s"  Rows removed: $malformedCount")
    ...
    dfReduced.select("title", "state", "formatted_work_type",
                     "formatted_experience_level",
    ...
    dfTransformed.select(
      ...
      "salary_source", "salary_source_idx_num",
      "normalized_salary", "remote_allowed"
    )
      .limit(10)
      .collect()
